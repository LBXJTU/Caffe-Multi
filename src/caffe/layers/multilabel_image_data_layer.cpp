...
void MultilabelImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  string filename;
  const int& multilabel_num = this->layer_param_.multilabel_image_data_param().multilabel_num();
  vector<float> multilabel(multilabel_num, 0);

  // read multi-dim labels
  while (infile >> filename) {
    for (int i = 0; i < multilabel_num; ++i) {
      infile >> multilabel[i];
    }
    //LOG(INFO) << "Filename" << filename <<" " << multilabel.size();
    lines_.push_back(std::make_pair(filename, multilabel));
  }
  ...
  LOG(INFO) << "output data size: " << top[0]->num() << ","
      ...
      << top[0]->width();
  // label
  //vector<int> label_shape(batch_size, multilabel_num, 1, 1);
...
void MultilabelImageDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    // read multidim float label from lines_
    const int& multilabel_num = this->layer_param_.multilabel_image_data_param().multilabel_num();
    for(int j = 0; j < multilabel_num; ++j){
      prefetch_label[item_id * multilabel_num + j] = lines_[lines_id_].second[j];
    }
